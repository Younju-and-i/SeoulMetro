...
const Map = () => {
  ...
  const startComparison = useCallback(() => {
    ...
    const requests = compareStations.map(s => 
        ...
        <section className="top-visual-row">
          ...
          <div className="summary-overlay-card">
            <div className="analysis-tabs">
              <button 
                className={`tab-btn ${analysisMode === 'single' ? 'active' : ''}`} 
                onClick={() => setAnalysisMode('single')}
              >
                단일 분석
              </button>
              <button 
                className={`tab-btn ${analysisMode === 'compare' ? 'active' : ''}`} 
              ...
              {analysisMode === 'single' ? (
                selectedStation ? (
                  <div className="summary-content">
                    <span className="line-tag" style={{ backgroundColor: LINE_COLORS[selectedStation.line] }}>
                ...
                <div className="compare-setup-panel">
                  ...
                  {compareStations.length >= 2 && (
                    <button className="start-btn" onClick={startComparison}>선택 항목 비교 분석 시작</button>
                  )}
                </div>
              )}
            </div>
